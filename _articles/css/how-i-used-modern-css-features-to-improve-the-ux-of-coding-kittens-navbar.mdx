...
I worked on many iterations for this site's navbar to achieve a look and feel that I would be happy with. I wanted a logo that would stand out and a navigation menu always visible — specially for mobile — so that you, the person reading this, could interact at all times with it.
...
The HTML and CSS is basic layout of a navbar. The interesting part is this JavaScript piece.
...
This code basically assigns a function handler to the global onscroll event and this function will check how much from the `<body>` or the `<html>` element the user has scrolled. It will apply one set of styles or the other based on the scroll offset.
...
What if I told you that there's a futuristic way of approaching this with only CSS? Would you take the <Pill>[#unstyled boring pill](#less-fun-implementation)</Pill> or the <Pill color="red">[#unstyled future pill](#scroll-function-and-scrolltimeline-explained)</Pill>?
...
On the other hand, the `animation-timeline:scroll` is less supported but we'll add a JavaScript fallback for it.

We'll start by explaining shortly what `animation-timeline:scroll` is.

## What is the Scroll CSS function and ScrollTimeline?

I'll be honest, when writting this I was going to explain what they are and how they work but I realized that everything I can say is properly explained in this sources where I found most of the information:
...
It means that this animations will not block the main thread, bye bye junky animations!
...
If you wonder what the `backdrop` `<div>` is, it's just acting as the navbar's blurry background.

Next, we'll need to add some styles for this elements to make them look like a navbar.
...
Once we have this pieces put together, we'll have something like this:
...
.navbar {
    ...
    animation-timeline: scroll(root block); /* [!code focus]*//* [!code ++] */
    animation-range-start: 0%; /* [!code focus]*//* [!code ++] */
    animation-range-end: 10%; /* [!code focus]*//* [!code ++] */
}
```
...
6. Then we specify that the range of this animation will be between `0%` and `10%` of the page scroll.
...
As I mentioned at the begining of this post, the `animation-timeline: scroll()` is not supported in all browsers. In fact as of January 2024 this is only supported in Chromium browsers and on Firefox under an experimental flag.

Fear nothing, I have the solution for you. We'll do some changes to our CSS and add a bit of JavaScript.

We'll start by adding the **CSS Supports at rule** to check if there's browser support for `animation-timeline: scroll()`
...
.navbar {
  ...
  @media (min-width: 768px) {
   ...
   --navbar-shrink: 1;/* [!code --] *//* [!code focus]*/
  /* [!code --] *//* [!code focus]*/
    animation: shrink;/* [!code --] *//* [!code focus]*/
    animation-timeline: scroll(root block);/* [!code --] *//* [!code focus]*/
    animation-range-start: 0%;/* [!code --] *//* [!code focus]*/
    animation-range-end: 10%;/* [!code --] *//* [!code focus]*/
...
If you want you can play with the result in this Code Sandbox:

<CodeSandbox files={
  getFiles({ animationStyles: true})
} />

## The CSS magic spell was cast almost perfect

There is an issue with this approach, I didn't find it at first but then I spotted it.

I encourage you to play with the Code Sandbox to see if you can find it.

The issue is that when there's no scrollable content the `animation-timeline: scroll()` triggers as if the content was scrolled completely.

I prepared the same Code Sandbox as before but without content. Check this:

<CodeSandbox files={
  getFiles({ animationStyles: true, noScrollableContent: true})
} />

You can see that the navbar has the `shrink` state by default!

I didn't find an official way to disable the `animation-timeline` trigger but I did find a trick to avoid it.

The navbar gets pinned to the top of the page when you scrolled past a certain amount. If there's no content the navbar doesn't get sticked to the top. So, I searched how to detect if an element gets pinned to the top when it's sticky and this is what I found:
...
Lets try applying this changes to our code.
...
First we add a negative top positioning to our `.navbar` element and we make sure to apply `animation-timeline:scroll()` only on the `.navbar` that has the `.is-pinned` class.
...
/* [!code focus]*/
    ...
    } else {/* [!code ++] *//* [!code focus]*/
      const observer = new IntersectionObserver(/* [!code ++] *//* [!code focus]*/
        ([e]) =>/* [!code ++] *//* [!code focus]*/
          e.target.classList.toggle("is-pinned", e.intersectionRatio < 1),/* [!code ++] *//* [!code focus]*/
        { threshold: [1] }/* [!code ++] *//* [!code focus]*/
      );/* [!code ++] *//* [!code focus]*/
/* [!code ++] *//* [!code focus]*/
      observer.observe(navbar);/* [!code ++] *//* [!code focus]*/
    }/* [!code ++] *//* [!code focus]*/
  })();/* [!code focus]*/
</script>/* [!code focus]*/
```

Then when `animation-timeline: scroll()` **IS** supported, we add an `IntersectionObserver` to it. This will add the `is-pinned` class if we pass through a minimal threshold.

You can check the result in this Code Sandbox:

<CodeSandbox files={
  getFiles({ animationStyles: true, topTrick: true, noScrollableContent: true })
} />

For a final version with the scrollable content check this Code Sandbox instead:
...
The goal to improve my users life was met, we discovered a way to apply a shrink animation to our navbar that is **more performant**, **extensible** and almost **CSS-only**.
