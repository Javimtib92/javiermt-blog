...
<Callout
title="Warning! Experimental feature" type="warning">At the time of writing this post **`animation-timeline:scroll`** feature is still experimental, so make sure you add a fallback. Check the compatibility list [here](https://caniuse.com/mdn-css_properties_animation-timeline_scroll)</Callout>
...
The navbar gets pinned to the top of the page when you scrolled past a certain amount. If there's no content, the navbar doesn't get stuck to the top. So, I searched how to detect if an element gets pinned to the top when it's sticky, and this is what I found:
...
Let's try applying these changes to our code.
...
The goal to improve my user's life was met, we discovered a way to apply a shrink animation to our navbar that is **more performant**, **extensible** and almost **CSS-only**.
