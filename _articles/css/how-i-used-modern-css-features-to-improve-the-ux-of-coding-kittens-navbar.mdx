...
The issue is that when there's no scrollable content the animation triggers as if the content was scrolled completely.
...
First we add a negative top positioning to our `.navbar` element and we make sure to apply the animation styles only on the `.navbar` that has the `.is-pinned` class.
...
Then if `animation-timeline` **IS** supported, we add an `IntersectionObserver` to it. This will add the `is-pinned` class if we pass through a minimal threshold.
